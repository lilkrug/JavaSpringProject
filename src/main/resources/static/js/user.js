...
async function genUserCreateButton(user , Scooter) {
    ...
    if (validateCard() && await isAuth()) {//&& isNotExist.ok
        ...
        await createUserOrder(data, token);

        errMes.innerHTML = 'Created';
        await genUserInfo();
    } else {
        errMes.innerHTML = 'Not all fields are correct';
    }
...
async function genCardCreate(user, comp) {
    ...
    create.innerHTML='';
    let createButton = buttonWithParams('ORDER');
...
async function getCertainCompUser(listProjectElement) {
    ...
    let comp = await userGetPhoneByName(listProjectElement['name'], token);
...
async function genUserInfo() {
    ...
    let userOrders = await getAllUserRentByUserId(textUserData['id'], token);
    ...
    for (let i = userRents.length - 1; i >= 0; i--) {
        ...
        for (let y = 0; y < 6; y++) {
            ...
            switch (y) {
                case 0: {
                    th.innerHTML = userOrders[i]['userName'];
                    break;
                }
                case 1: {
                    th.innerHTML = userOrders[i]['userSurname'];
                    break;
                }
                case 2: {
                    th.innerHTML = userOrders[i]['scooter']['name'];
                    break;
                }
                case 3: {
                    th.innerHTML = userOrders[i]['phone']['orderDate'];
                    break;
                }
                case 4: {
                    th.innerHTML = userOrders[i]['rent'];
                    break;
                }
                case 5: {
                    ...
                    genBut.onclick = async () => {
                        async function deleteComp(userRentElement) {
                            ...
                            await deletePhoneByNameU({name:userRentElement['userName']},token);
                            await genUserInfo();
